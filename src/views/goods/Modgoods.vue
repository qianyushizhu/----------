...
import { getUploadToken, getClassigoods, Modgoods } from "@/request/http.js";
export default {
  ...
  created() {
    ...
    that.form.name1 = that.List.goodsName;
    that.form.img1 = [
      {
        name: "ban1.jpg",
        response: {
        ...
        url: that.img + that.List.logo,
      },
    ];
    let goodList = [];
    that.List.imgs.split(",").forEach((x, y) => {
      goodList.push({
        name: "y.jpg",
        response: {
          key: x,
    ...
    that.form.img2 = goodList;
    let attributeList = that.List.attributeArr;
    if (that.List.attributeArr.length > 0 || that.List.attributeArr) {
    ...
    that.tableData3 = that.List.goodsTypeArr;
    that.selectList();
  },
  methods: {
    //返回
    ...
    async selectList() {
      ...
      let tableData3 = that.tableData3;
      ...
      if (tableData3) {
        tableData3.forEach((x) => {
          List.forEach((j) => {
            if (x.typeId == j.typeId) {
              j.key = true;
            }
          });
        });
