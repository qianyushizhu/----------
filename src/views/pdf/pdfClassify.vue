  ...
  data() {
    return {
      ...
      tableSetting: {
        ...
        urlName: "/classify/getPdfClassify", //请求url
  ...
  methods: {
    ...
    async confirm() {
      ...
      if (name == "") {
        ...
        let res = await apiInsPdfClassify({ name });
        console.log(res);
        //重新定义接口请求参数
    ...
    async confirm1() {
      ...
      let res = await apiUpPdfClassify({ id, name });
      //重新定义接口请求参数
      that.tableSetting.params = { page: 1, pageSize: 10 };
      //再次请求接口
      that.$refs.resTable.reload();
    ...
    async Delete(row) {
      ...
      that.$messageBox
        ...
        .then(async () => {
          let classifyID = `${row.id}`;
          let res = await apiDelPdfClassify({ classifyID });
          //重新定义接口请求参数
          that.tableSetting.params = { page: 1, pageSize: 10 };
          //再次请求接口
          that.$refs.resTable.reload();
    ...
    async ensure() {
      ...
      let res = await setpdfWeight({ id, weight });
      console.log(res);
      //重新定义接口请求参数
      that.tableSetting.params = { page: 1, pageSize: 10 };
      //再次请求接口
