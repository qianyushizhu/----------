...
import { ModHotvideo, getUploadToken } from "@/request/http";
...
export default {
  ...
  created() {
    ...
    that.form.name1 = that.List.name;
    that.form.name2 = that.List.detail;
    that.form.img1 = cture(that.List.image);
    //视频
    if (that.List.type == 1) {
      that.radio = 0;
      that.route = that.List.route;
    }
    //链接
    else if (that.List.type == 2) {
      that.radio = 1;
      that.input3 = that.List.route;
    }
  },
  methods: {
    //返回
    ...
    async apiinsert() {
      ...
      if (
        ...
        let res = await ModHotvideo(data);
        console.log(res);
        that.$message({
          message: "提交成功",
          type: "success",
