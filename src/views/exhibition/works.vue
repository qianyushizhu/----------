...
<script>
import { delworks, Avworks, setworks } from "../../request/http.js";
...
export default {
  ...
  data() {
    return {
      ...
      tableSetting: {
        ...
        urlName: "/opusOpen/selectList", //请求url
  ...
  methods: {
    ...
    Newactive() {
      let that = this;
      that.$router.push({ name: "Addworks", params: {} });
    },
    //修改
    Modify(row) {
      let that = this;
      that.$router.push({ name: "Modworks", params: { row } });
    },
    //删除
    async Delete(row) {
      let that = this;
    ...
    async ensure() {
      ...
      let res = await setworks({ id, weight });
      console.log(res);
      //重新定义接口请求参数
      that.tableSetting.params = { page: 1, pageSize: 10 };
      //再次请求接口
    ...
    viewgoods(row) {
      ...
      sessionStorage.setItem("worksList", List);
      that.$router.push({ name: "viewgoods", params: { row } });
    },
  },
};
</script>
