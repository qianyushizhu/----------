...
import { delHotvideo, AvHotvideo, setHotvideo } from "@/request/http";
export default {
  ...
  data() {
    return {
      ...
      tableSetting: {
        ...
        urlName: "/smallVideo/selectList", //请求url
  ...
  methods: {
    ...
    Newactive() {
      let that = this;
      that.$router.push({ name: "AddShotalkabout", params: {} });
    },
    //删除
    async Delete(row) {
      let that = this;
      that.$messageBox
        ...
        .then(async () => {
          let idList = `${row.id}`;
          let res = await delHotvideo({ idList });
          console.log(res);
          //重新定义接口请求参数
    ...
    Modify(row) {
      let that = this;
      that.$router.push({ name: "ModShotalkabout", params: { row } });
    },
    //上架或下架
    async setStatus(row) {
      let that = this;
      let id = row.id;
      let res = await AvHotvideo({ id });
      //重新定义接口请求参数
      that.tableSetting.params = { page: 1, pageSize: 10 };
      //再次请求接口
      that.$refs.resTable.reload();
    ...
    async ensure() {
      ...
      let res = await setHotvideo({ id, weight });
      console.log(res);
      //重新定义接口请求参数
      that.tableSetting.params = { page: 1, pageSize: 10 };
      //再次请求接口
